...
# Filter rows where 'Accuracy (m)' is under 100 meters
df = df[df['Accuracy (m)'] < 100]

# Print the first few rows to confirm data is loaded and filtered correctly
print("Data loaded and filtered from CSV (Accuracy < 100 m):")
print(df.head())

# Create a new KML object and set the document name
file_name_without_extension = os.path.splitext(file_name)[0]
kml = simplekml.Kml()
kml.document.name = f"{output_suffix} - Filtered Locations (Accuracy < 100 m) - {file_name_without_extension}"
